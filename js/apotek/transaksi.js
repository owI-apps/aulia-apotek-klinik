...
window.AppApotekTransaksi = {
    ...
    init: function() {
        ...
        Promise.all([pObat, pConfig, pResep, pAntrian]).then(function(results) {
            ...
            results[0].forEach(function(doc) { var d = doc.data(); d.id = doc.id; AppApotekTransaksi.masterObat.push(d); });
        ...
        }).catch(function(err) { Utils.toast('Gagal memuat data: ' + err.message, 'error'); });
    },
    ...
    hitungTotal: function() {
        ...
        rows.forEach(function(row) {
            ...
            var sub = qty * harga;
            document.getElementById('trx-sub-' + idx).textContent = Utils.formatRapiah(sub);
            totalObat += sub;
    ...
    simpan: function() {
        ...
        db.collection('transaksi').add(obj).then(function(docRef) {
            ...
            items.forEach(function(item) {
                if (item.jumlah > 0) {
                    db.collection('obat').doc(item.obatId).update({ stok: firebase.firestore.FieldValue.increment(-item.jumlah) });
                }
            });

            if (this.tipe === 'resep_klinik' && obj.resepId) {
